...
toggle_btns.map(toggle_btn => {
    toggle_btn.addEventListener('click', toggleTarefa)
})
...
function montarItem(id, feito, nome, desc) {
    ...
    footer.classList.add('line', 'tarefa-footer', 'control-btns')
    footer.appendChild(btnDelete)
    footer.appendChild(btnEdit)
    // Criar Seção
    const section = document.createElement('div')
    section.classList.add('line', 'tarefa-section')
    section.id = `tarefa-${id}.descricao`
    section.innerHTML = desc
    // Criar Body
    const body = document.createElement('div')
    body.id = `tarefa-${id}.body`
    body.classList.add('hidden', 'tarefa-body')
